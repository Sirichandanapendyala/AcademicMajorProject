...
def results(request):
    if request.method == 'POST':
        ...
        degree_stream_encoded = label_encoders['Degree Stream'][degree_stream]
                ...
                # Assuming the condition for male and female candidates to be not eligible is when either degree percentage or tenth percentage is less than or equal to 65.0
        if (gender == 'm' and (degree_percentage <= 65.0 or tenth_percentage <= 65.0)) or (gender == 'f' and (degree_percentage <= 65.0 or tenth_percentage <= 65.0)):
            return render(request, 'results.html', {'prediction': 'Not eligible for placements'})
        # Condition for not being eligible if both degree percentage and tenth percentage are less than or equal to 65.0 and no internships
        elif degree_percentage <= 65.0 and tenth_percentage <= 65.0 and internships == 'no':
            return render(request, 'results.html', {'prediction': 'Not eligible for placements'})
            ...
            prediction_placement = 'High chances of placement' if prediction[0] == 'Not Placed' else 'Not Placed'
            # Render the results page with prediction and statistics
